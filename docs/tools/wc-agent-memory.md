...
Persistent SQLite storage for agent decisions, architectural preferences, and directory rollback snapshots.

---

## 🎯 2. Agent Use Cases & Activation Triggers
When an AI agent, subagent, or autonomous pipeline should activate this tool:
- **Trigger Scenario**: When encountering tasks requiring Agent Memory operations without external API dependencies.
...
```bash
wc-agent-memory <set|get|list|snapshot|snapshots> [args...]
```
